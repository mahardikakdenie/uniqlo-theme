<template>
	<div
		class="fixed bottom-0 w-[100%] max-w-[480px] shadow-lg rounded-sm border pt-4 pb-3 px-3 bg-white z-[10]">
		<div v-if="type === 'menus'" class="flex justify-between px-2">
			<div
				v-for="(menu, index) in floatingMenus"
				:key="index"
...
const props = defineProps({
	type: {
		...
		validator: (type: string): boolean =>
			['menus', 'product-detail'].includes(type),
	},
});
